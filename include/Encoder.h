...
public:
    ...
    // Open an output file for encoding. srcVideoTimeBase is the time base of the
    // incoming video frames so PTSes can be rescaled to the encoder's time base.
...
private:
    ...
    AVFrame*         m_swrFrame    = nullptr; // S16 → FLTP resampling buffer
    AVFrame*         m_encFrame    = nullptr; // fixed-size frame fed to the audio encoder
