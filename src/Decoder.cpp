...
bool Decoder::initAudioCodec() {
    ...
    if (avcodec_open2(m_codecCtxAudio, codec, nullptr) < 0) return false;
...
void Decoder::close() {
    ...
    if (m_codecCtxVideo) { avcodec_free_context(&m_codecCtxVideo); }
    if (m_codecCtxAudio) { avcodec_free_context(&m_codecCtxAudio); }
...
bool Decoder::readFrame(DecodedFrame& out) {
    ...
    while (av_read_frame(m_fmtCtx, m_packet) >= 0) {
        ...
        if (isVideo)      ctx = m_codecCtxVideo;
        else if (isAudio) ctx = m_codecCtxAudio;

        if (!ctx) { av_packet_unref(m_packet); continue; }
        ...
        if (avcodec_receive_frame(ctx, m_frame) >= 0) {
            ...
            if (isVideo) {
                ...
                // Resample to S16 interleaved stereo 44100 Hz.
...
bool Decoder::seek(double seconds) {
    ...
    if (av_seek_frame(m_fmtCtx, -1, ts, AVSEEK_FLAG_BACKWARD) < 0)
        return false;
    if (m_codecCtxVideo) avcodec_flush_buffers(m_codecCtxVideo);
    if (m_codecCtxAudio) avcodec_flush_buffers(m_codecCtxAudio);
    // Drain any samples buffered inside the resampler.
    if (m_swrCtx) swr_convert(m_swrCtx, nullptr, 0, nullptr, 0);
    return true;
}
